...
public class CodexAPI {
	...
	public static boolean hasUnlockedDiscovery(Player player,String category,String discovery) {
		JugadorCodex j = plugin.getJugadorDataManager().getJugadorSync(player.getUniqueId());
		if(j != null) {
	...
	public static int getTotalDiscoveries(Player player, String category) {
		...
		if(j != null) {
			if(category == null) {
				return j.getEntradasDesbloqueadas(categoriasManager);
			}else {
				return j.getEntradasDesbloqueadas(categoriasManager.getCategoria(category));
			}
		}
		return 0;
	}
