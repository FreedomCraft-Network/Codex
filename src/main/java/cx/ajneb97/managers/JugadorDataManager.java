...
public class JugadorDataManager {
	...
	public JugadorCodex getJugadorSync(UUID uuid) {
		...
		if(MySQL.isEnabled(plugin.getConfig())) {
			...
			for(JugadorCodex j : jugadores) {
				if(j.getName() != null && j.getUuid().equals(uuid)) {
					jugadorCodex = j;
					break;
				}
			}
	...
	public void getJugador(final UUID uuid, final JugadorCodexCallback callback) {
		new BukkitRunnable() {
			@Override
			public void run() {
	...
	public void agregarEntrada(final Player jugador, final String categoria, final String discovery,final AgregarEntradaCallback callback) {
		getJugador(jugador.getUniqueId(), new JugadorCodexCallback() {
			@Override
			public void onDone(JugadorCodex j) {
				boolean agrega = false;
				if(MySQL.isEnabled(plugin.getConfig())) {
					if(j == null) {
						j = new JugadorCodex(jugador.getUniqueId(),jugador.getName());
