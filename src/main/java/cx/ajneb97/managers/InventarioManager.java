...
public class InventarioManager {
	...
	public void crearInventario(final String nombre,final Player jugador) {
		...
		jugadorManager.getJugador(jugador.getUniqueId(), new JugadorCodexCallback() {
			@Override
						...
						if(tipo != null) {
							if(tipo.contains(";")) {
								...
								if(categoriaCodex != null) {
									...
									if(j != null) {
										entradasDesbloqueadas = j.getEntradasDesbloqueadas(categoriaCodex);
									}
	...
	public void clickearItemInventario(ItemInventarioCodex itemCodex,Player jugador) {
		...
		if(itemCodex.getTipo() != null) {
			...
			if(tipo.length == 2) {
				...
				JugadorCodex j = plugin.getJugadorDataManager().getJugadorSync(jugador.getUniqueId());
